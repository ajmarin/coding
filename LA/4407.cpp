...
#ifdef DEBUG
		printf("Tower (%d,%d,%d) is delimiter\n",t[a].x,t[a].y,t[a].p);
		printf("Tower (%d,%d,%d) is delimiter\n",t[b].x,t[b].y,t[b].p);
#endif
		for(int i = 0; i < N; ++i){
			if(i == a || i == b) continue;
...
#ifdef DEBUG
		for(int i = 0; i < N; ++i){
			printf("Tower (%d,%d,%d) team %d\n",t[i].x,t[i].y,t[i].p,t[i].team);
		}
#endif
		int AA, BB;
...
#ifdef DEBUG
		printf("Team A\n");
		for(int i = 0; i < AA; ++i) printf("Tower (%d,%d,%d)\n",t[i].x,t[i].y,t[i].p);
		printf("Team B\n");
		for(int i = AA; i < N; ++i) printf("Tower (%d,%d,%d)\n",t[i].x,t[i].y,t[i].p);
#endif
		for(int i = 0; i < AA; ++i)
			if(t[i].p != 0)
				for(int j = AA; j < N; ++j)
...
#ifdef DEBUG
						printf("Tower (%d,%d,%d) can kill (%d,%d,%d)\n",
								t[i].x,t[i].y,t[i].p,t[j].x,t[j].y,t[j].p);
#endif
						t[i].ck.push_back(j);
					}
		printf("%d\n",hopcroft_karp(AA,BB));
